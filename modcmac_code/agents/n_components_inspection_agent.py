...
class n_components_inspection_agent:
    def __init__(self, env, utility, percentage=0.5, sample=False):
        self.env = env
    ...
    def check_failing(self, obs):
        n_fail = (obs == 4).sum()
        # print(n_fail, self.n_comp, self.percentage, obs)

        if n_fail / self.n_comp >= self.percentage:
    ...
    def do_test(self, n_times=5, n_steps=50):
        ...
        for i in range(n_times):
            reward, n, epi = self.do_run(n_steps)
            length[i] = n
            rewards[i] = reward

...
from ..networks.model import PNet as PnetOrig, VNetDCMAC as VnetOrig
import numpy as np
...
class MODCMAC_SER:
    ...
    def __init__(self, env: Env, ncomp: int, nstcomp: int, nacomp: int, naglobal: int,
                 ...
                 save_folder: str = "./models", use_lr_scheduler: bool = False, num_episodes: int = 500_000,
                 ...
                 use_accrued_reward: bool = True, n_eval: int = 100):
        ...
        self.name = "test_{name}_{date}".format(name=name, date=datetime.now().strftime("%Y%m%d-%H%M%S"))
        ...
        for ep in range(0, episodes + 1):
            ...
            while not done:
                ...
                if (total_steps + 1) % self.n_step_update == 0:
                    loss_p, loss_v, entropy, update_loss = self.learn(total_steps=total_steps)
                    mean_policy_loss += loss_p
                    mean_value_loss += loss_v
            ...
            if (ep + 1) % 100 == 0:
                ...
                print(f'Episode {ep + 1}/{episodes} | '
                      ...
                      f'Policy Loss {(mean_policy_loss / n_updates):.2f} | '
                      f'Value Loss {(mean_value_loss / n_updates):.2f} | '
                      f'Entropy {(mean_entropy / n_updates):.2f}', file=sys.stderr)
                mean_entropy = 0
                mean_value_loss = 0
                mean_policy_loss = 0
                n_updates = 0
    ...
    def learn(self, total_steps=0) -> Tuple[float, float, float, int]:
        ...
        with torch.no_grad():
            value_next = self.Vnet(batch.belief_next)

        value_curr = self.Vnet(batch.belief)
        ...
        returns[-1] += self.gamma * value_next[-1] * non_terminal[-1]
        ...
        critic_loss = (returns - value_curr).pow(2).sum(dim=1)

        # print(critic_loss.shape)
        # print(critic_loss)
        # exit()
        self.writer.add_scalar("Debugging/Critic_loss (Cost)", critic_loss[0], total_steps)
        self.writer.add_scalar("Debugging/Critic_loss (Risk)", critic_loss[1], total_steps)
        self.writer.add_scalars("Debugging/Critic_loss", {"Cost": critic_loss[0], "Risk": critic_loss[1]}, total_steps)
        self.writer.add_histogram("Debugging/Cost Value", value_curr[:, 0], total_steps)
        self.writer.add_histogram("Debugging/Risk Value", value_curr[:, 1], total_steps)
        self.writer.add_histogram("Debugging/Risk Returns", returns[:, 1], total_steps)
        ...
        for i in range(self.n_objectives):
            exp_var_arr[i] = explained_variance(value_curr[:, i].detach().numpy(), returns[:, i].numpy())
            # print(exp_var_arr[i])
            # if exp_var_arr[i] == nan:
            #     no_nan = False
            #     print("Komt hier")
        # if mean_exp_var
        if not np.isnan(exp_var_arr).any():
            self.writer.add_scalar('Debugging/Explained_variance', np.mean(exp_var_arr), total_steps)
            self.writer.add_scalar('Debugging/Explained_variance (Risk)', exp_var_arr[1], total_steps)
        self.writer.add_scalar('Debugging/Explained_variance (Cost)', exp_var_arr[0], total_steps)

        self.writer.add_scalar('Debugging/Advantage', torch.mean(advantage), total_steps)
        # self.writer.add_scalar('Debugging/Advantage_std', torch.std(advantage), total_steps)
        # self.writer.add
